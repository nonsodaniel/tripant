...
const CATEGORIES: {
  ...
  { category: "food",       label: "Food & Drink",  emoji: "🍽️", description: "Restaurants & cafes" },
        ...
        {CATEGORIES.map(({ category, label, emoji, description }) => {
          ...
          return (
            <Link
              key={category}
              href={`/search?${params}`}
              className="group flex flex-col items-center gap-2 p-3 pt-4 bg-surface border border-border rounded-2xl
                         hover:border-accent hover:bg-accent-light
                         active:scale-[0.95] active:bg-accent-light
                         transition-all duration-150 select-none"
            >
              <span
                className="text-3xl leading-none group-hover:scale-110 transition-transform duration-150"
                role="img"
                aria-label={label}
              >
                {emoji}
